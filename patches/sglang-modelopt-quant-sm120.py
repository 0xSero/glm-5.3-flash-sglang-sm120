...
class ModelOptNvFp4FusedMoEMethod(FusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # The serialized GLM checkpoint carries activation scales for all
        # experts, while EP shards the FP4 expert weights and their global
        # weight scales.  Keep the activation scales aligned with the local
        # expert interval before combining them.  The upstream CuteDSL branch
        # already performs this slice; DeepGEMM and the generic path did not.
        def _slice_global_expert_scale(scale):
            if (
                scale.dim() > 0
                and layer.moe_ep_size > 1
                and scale.shape[0] == layer.num_experts
            ):
                start = layer.moe_ep_rank * layer.num_local_experts
                return scale[start : start + layer.num_local_experts]
            return scale

        w13_input_scale = _slice_global_expert_scale(w13_input_scale)
        w2_input_scale = _slice_global_expert_scale(w2_input_scale)
