...
class DeepseekModelNextN(nn.Module):
    ...
    def __init__(
        ...
        if quant_config is not None and quant_config.get_name() == "modelopt_fp4":
            # Legacy DeepSeek ModelOpt checkpoints store their NextN block in
            # BF16, so SGLang historically discarded the target quant config
            # here. GLM-5.3-Flash stores layer 45's routed experts as packed
            # NVFP4 (with weight_scale and weight_scale_2); dropping the config
            # allocates BF16 [4096, ...] expert parameters and then attempts to
            # copy the packed uint8 [2048, ...] payload into them. Preserve the
            # ModelOpt config for GLM-5.3 so the draft uses the same physical
            # NVFP4 representation as the target.
            if not str(getattr(config, "model_type", "")).startswith("glm5_next"):
                logger.warning(
                ...
                logger.warning(
                    "Keeping modelopt_fp4 quant config for GLM5 NextN NVFP4 experts."
                )
