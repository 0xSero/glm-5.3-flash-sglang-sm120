...
def _module_path_match(ignored: str, prefix: str) -> bool:
    ...
    ignored_variants = [ignored]
    if ignored.startswith("model.language_model."):
        ignored_variants.append(
            "model." + ignored.removeprefix("model.language_model.")
        )
    if ignored.startswith("model."):
        ignored_variants.append(ignored.removeprefix("model."))
    if ignored.startswith("language_model."):
        ignored_variants.append(ignored.removeprefix("language_model."))
    # ModelOpt emits shell-style module globs (for example,
    # ``*.self_attn.q_proj`` and ``model.visual.*``).  The stock matcher
    # treats ``*`` literally, which causes SGLang's fused qkv modules to be
    # quantized even when every source shard is explicitly excluded.  That
    # changes the parameter storage shape and makes BF16 checkpoint weights
    # impossible to load.  Match globs against the complete module path.
    for candidate in dict.fromkeys(ignored_variants):
        if "*" in candidate:
