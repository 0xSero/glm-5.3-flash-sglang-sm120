...
_GLM_DSA_MODEL_ARCHS = (
    ...
    # GLM-5.3-Flash uses the same SGLang DSA contract (FP8 KV on SM120/121)
    # under the new multimodal conditional-generation architecture name.
...
@triton.jit
def _compact_nope_kv_to_h576_kernel(
    src_ptr,
    indices_ptr,
    dst_ptr,
    remapped_ptr,
    ...
    """Gather H512 sparse rows and append a zero BF16 RoPE lane on GPU."""
    entry = tl.program_id(0)
    if entry >= num_entries:
        return
    src_token = tl.load(indices_ptr + entry)
    valid = src_token >= 0
    tl.store(remapped_ptr + entry, tl.where(valid, entry, -1))
    offsets = tl.arange(0, block_bytes)
    values = tl.load(
        src_ptr + src_token.to(tl.int64) * src_stride + offsets,
        mask=valid & (offsets < copy_bytes),
        other=0,
    )
    # Bytes [0:528] are FP8 KV + four FP32 scales. Bytes [528:656]
    # represent 64 BF16 RoPE values and stay exactly zero.
    tl.store(
        dst_ptr + entry.to(tl.int64) * dst_stride + offsets,
        values,
        mask=offsets < dst_stride,
    )


def _flashinfer_nope_via_zero_rope(
    *,
    q: torch.Tensor,
    ...
    """Run GLM-5.3 H512 DSA through the exact H576 SM120 kernel ABI.

    The shipped SM120 kernel's V path consumes only the 512 latent values.
    Appending a zero RoPE lane therefore preserves both QK and V exactly while
    retaining its optimized sparse attention implementation.
    """
    from flashinfer.mla import trtllm_batch_decode_with_kv_cache_mla
    from sglang.srt.runtime_context import get_resources

    assert kv_cache_dim == kv_lora_rank + 16 == 528
    indices = indices.contiguous()
    original_topk = indices.shape[1]
    aligned_topk = ((original_topk + 3) // 4) * 4
    if aligned_topk != original_topk:
        aligned = torch.full(
            (indices.shape[0], aligned_topk),
            -1,
            dtype=indices.dtype,
            device=indices.device,
        )
        aligned[:, :original_topk].copy_(indices)
        indices = aligned

    num_queries = q.shape[0]
    # Keep compaction memory bounded for real prefills while covering the
    # largest decode/verify CUDA graph (8 requests x 6 draft tokens) in one
    # launch. Each compacted row is only the selected top-k, never the full KV.
    queries_per_chunk = 64
    reserve_entries = max(64 * 4096, min(num_queries, queries_per_chunk) * aligned_topk)
    reserve_entries = ((reserve_entries + page_size - 1) // page_size) * page_size
    resources = get_resources().buffers
    device = q.device
    cache_key = f"glm53_nope_h576_cache:{device}"
    index_key = f"glm53_nope_h576_indices:{device}"
    compact_cache = resources.get(cache_key)
    compact_indices = resources.get(index_key)
    if compact_cache is None or compact_cache.numel() < reserve_entries * 656:
        with torch.inference_mode(False):
            compact_cache = torch.empty(
                reserve_entries * 656,
                dtype=torch.uint8,
                device=device,
            )
        resources[cache_key] = compact_cache
    if compact_indices is None or compact_indices.numel() < reserve_entries:
        with torch.inference_mode(False):
        ...
        resources[index_key] = compact_indices

    src = kv_cache.view(torch.uint8).reshape(-1)
    result = torch.empty(
        (num_queries, q.shape[1], kv_lora_rank),
        dtype=torch.bfloat16,
        device=device,
    )
    for start in range(0, num_queries, queries_per_chunk):
        end = min(start + queries_per_chunk, num_queries)
        real_queries = end - start
        q_chunk = q[start:end]
        q_h576 = torch.cat(
            [q_chunk, torch.zeros_like(q_chunk[..., :64])], dim=-1
        )

        def run_segment(segment_indices: torch.Tensor, kernel_topk: int):
            if segment_indices.shape[1] != kernel_topk:
                padded = torch.full(
                    (real_queries, kernel_topk),
                    -1,
                    dtype=indices.dtype,
                    device=device,
                )
                padded[:, : segment_indices.shape[1]].copy_(segment_indices)
                segment_indices = padded
            segment_indices = segment_indices.contiguous()
            entries = segment_indices.numel()
            padded_entries = ((entries + page_size - 1) // page_size) * page_size
            remapped = compact_indices[:entries].view(real_queries, kernel_topk)
            _compact_nope_kv_to_h576_kernel[(entries,)](
                src,
                segment_indices,
                compact_cache,
                remapped,
            ...
            cache_view = compact_cache[: padded_entries * 656].view(
                padded_entries // page_size, 1, page_size, 656
            )
            active_lens = (segment_indices >= 0).sum(dim=1, dtype=torch.int32)
            return trtllm_batch_decode_with_kv_cache_mla(
                query=q_h576.unsqueeze(1),
                kv_cache=cache_view,
                workspace_buffer=workspace_buffer,
                qk_nope_head_dim=qk_nope_head_dim,
                kv_lora_rank=kv_lora_rank,
                qk_rope_head_dim=64,
                block_tables=remapped.unsqueeze(1),
                seq_lens=active_lens,
        ...
        # GLM emits index_topk (2048) plus index_kpool (4).  FlashInfer's
        # SM120 GLM decoder instantiates top-k 2048 and 128, but not 2052.
        # Evaluate both exact sets and merge with their returned LSE values.
        main_out, main_lse = run_segment(indices[start:end, :2048], 2048)
        tail_out, tail_lse = run_segment(indices[start:end, 2048:], 128)
        merged_lse = torch.logaddexp(main_lse, tail_lse)
        main_weight = torch.exp(main_lse - merged_lse)[:, None, :, None]
        tail_weight = torch.exp(tail_lse - merged_lse)[:, None, :, None]
        merged = main_out * main_weight + tail_out * tail_weight
        result[start:end].copy_(merged.squeeze(1))
    return result
...
def flashinfer_sparse_mla_forward(
    ...
    *,
    ...
    if qk_rope_head_dim == 0:
        return _flashinfer_nope_via_zero_rope(
            q=q,
            kv_cache=kv_cache,
            indices=indices,
            seq_lens=seq_lens,
    ...
    kv_full = kv_cache.view(-1, page_size, kv_cache_dim)
    if qk_rope_head_dim == 0 and kv_cache_dim == kv_lora_rank + 16:
        # SGLang's H512 FP8 cache is physically [512 data bytes, 16 inline
        # FP32 scale bytes]. FlashInfer's public shape contract describes only
        # the logical 512-wide KV vector; its native dynamic-sparse kernel uses
        # the 528-element token stride to reach the appended scales. Keep the
        # storage and strides intact while hiding the scale footer dimension
        # from the Python/C++ head-dimension validators.
        kv = kv_full.as_strided(
            (kv_full.shape[0], 1, page_size, kv_lora_rank),
            (kv_full.stride(0), kv_full.stride(0), kv_full.stride(1), 1),
        )
    else:
        kv = kv_full.unsqueeze(1)
